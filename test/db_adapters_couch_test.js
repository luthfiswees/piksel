...
const adapter = require('../db/connection').database;

// Test Objects
const testDbName = 'piksel';
...
describe('CouchDB Database', () => {
    describe('Database creation and teardown', () => {
        it ("should have created a db", async () => {
            let stub = sinon.stub(adapter, 'createDatabase').resolves();
        ...
        it ("should have dropped a db", async () => {
            let stub = sinon.stub(adapter, 'dropDatabase').resolves();
        ...
        it('should have return error message if database is uncontactable', async () => {
            ...
            let stubCreate = sinon.stub(adapter, 'createDatabase').rejects(rejectBody);
            let stubDrop   = sinon.stub(adapter, 'dropDatabase').rejects(rejectBody);
    ...
    describe('Database Operation', () => {
        describe("Store", () => {
            it('should be able to store object with keys', async () => {
                let stub = sinon.stub(adapter, "insert").callsFake(() => {
            ...
            it('should be able to store and update automatically if entry already exist', async () => {
                let stubInsert = sinon.stub(adapter, "insert").callsFake(() => {
                    return Promise.reject(rejectBodyBySameEntry);
                })
                let stubUpdate = sinon.stub(adapter, "update").callsFake(() => {
                    return Promise.resolve(resolveBody)
                })
                let stubGet = sinon.stub(adapter, "get").callsFake(() => {
                    return Promise.resolve(resolveBody)
                })
        ...
        describe("Get", () => {
            it('should be able to retrieve available data', async () => {
                let stub = sinon.stub(adapter, "get").callsFake(() => {
                    return Promise.resolve(resolveBody)
                })
        ...
        describe("Erase", () => {
            it("should be able to erase existing entry", async () => {
                let stubDelete = sinon.stub(adapter, "del").callsFake(() => {
                    return Promise.resolve(resolveBody)
                })
                let stubGet = sinon.stub(adapter, "get").callsFake(() => {
                    return Promise.resolve(resolveBody)
